...
def run_stress_test():
    ...
    try:
        ...
        for line in siege_output.split('\n'):
            ...
            if '"availability":' in line_lower:
                print(f"{GREEN}{line}{RESET}")
                try: availability = float(line_lower.split(':')[1].replace(',', '').strip())
                except ValueError: pass
            elif "availability:" in line_lower:
                print(f"{GREEN}{line}{RESET}")
                try: availability = float(line.split()[1].replace('%', ''))
                except ValueError: pass
            elif "failed responses" in line_lower or "longest transaction" in line_lower or '"failed_transactions":' in line_lower:
                print(line)
        print("---------------------\n")
